...
public class ChangeLogEntryVisitor extends BaseChangeLogEntryVisitor {
		...
		if (!initialLoadDone) {
			try {
				...
				throw new RuntimeException(
						"Not a valid sandbox. Please run [scm load "
								+ workspace
	...
	private void acceptAndLoadBaseline(String baselineItemId) {
		// Baselines could not be successfully accepted by rtc cli, therefore do
		// not do it
		// new AcceptCommandDelegate(config, workspace, baselineItemId,
		// true).run();
		handleInitialLoad();
	}

	private void acceptAndLoadChangeSet(String changeSetUuid)
			throws CLIClientException {
		int result = new AcceptCommandDelegate(config, workspace,
				changeSetUuid, false, false).run();
		if (Constants.STATUS_GAP == result) {
			stdout().println("Retry accepting with --accept-missing-changesets");
			result = new AcceptCommandDelegate(config, workspace,
					changeSetUuid, false, true).run();
			if (Constants.STATUS_GAP == result) {
				throw new CLIClientException(
						"There was a GAP in accepting that we cannot resolve.");
			}
